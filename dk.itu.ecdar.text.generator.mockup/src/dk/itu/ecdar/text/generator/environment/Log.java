...
public class Log {
	...
	public static synchronized Log getInstance() {
		if (_instance == null) {
			_instance = new Log();
			_instance.output = Output.CONSOLE;
	...
	public static void log(String s) {
		if (getInstance().output == Output.CONSOLE) {
			getInstance().logToConsole(s);
		} else {
			getInstance().logToFile(s);
		}
	}
	...
	private void logToFile(String s) {
		try {
			...
			// Create file if it does not exist and make sure to not overwrite
			// data
